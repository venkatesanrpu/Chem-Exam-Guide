...
def process_image_file(image_path, github_repo):
    ...
    # Expect path like: problem-set-1/images/reaction_type/image.png
    if len(path_parts) < 4 or path_parts[-3] != 'images':
        print(f"DEBUG: Skipping invalid path structure: {image_path}")
        return None
    
    problem_folder = path_parts[-4]  # e.g., problem-set-1
    reaction_type = path_parts[-2]   # e.g., addition_reaction
    ...
    # Construct URL with images/reaction_type
    question_url = f"https://{repo_owner}.github.io/{repo_name}/{problem_folder}/images/{reaction_type}/{image_name}"
    
    entry = {
        "question_url": question_url,
        "question_level": reaction_type,  # Using reaction_type as level
        "question_category": reaction_type,
        "question_text": "Solve the problem ..."
    }
    
    print(f"DEBUG: Created entry: {entry}")
    return problem_folder, reaction_type, entry
...
def main():
    ...
    for file in changed_files:
        ...
        if result:
            problem_folder, reaction_type, entry = result
            json_file = f"{problem_folder}/{reaction_type}.json"
